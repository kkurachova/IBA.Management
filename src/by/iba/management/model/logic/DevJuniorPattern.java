...
public class DevJuniorPattern {
    ...
    private boolean isPhp = ProgrammingLanguage.isPhp(false);
    private boolean isDotNet = ProgrammingLanguage.isDotNet(false);
    private boolean isSql = Skills.isSql(true);
    private boolean isJavaScript = Skills.isJavaScript(false);
    private boolean isHtml = Skills.isHtml(true);
    private boolean isCss = Skills.isCss(false);
    private boolean isjQuery = Skills.isjQuery(false);
    private boolean isVisualStudio = Tools.isVisualStudio(true);
    private boolean isIntellijIdea = Tools.isIntellijIdea(false);
    private boolean isEclipse = Tools.isEclipse(false);
    private boolean isNetBeans = Tools.isNetBeans(false);

    public DevJuniorPattern(String l_level, boolean l_teamLead, String l_englishLanguageLevel,
